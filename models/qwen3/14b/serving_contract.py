...
def bind_qwen3_kernel_functions(
    *,
    ...
    globals()["token_embed_fwd"] = token_embed_fwd
...
def qwen3_token_embed_host(
    ...
    return token_embed_fwd(sampled_ids, embed_weight, next_hidden)
...
def build_token_embed_runtime_args(
    inputs: Any,
    static: Any,
    *,
    next_hidden_buffer: Any,
) -> tuple[Any, ...]:
    """Build arguments in qwen3_token_embed_host signature order."""
    return (inputs.sampled_ids, static.padded_embed_weight, next_hidden_buffer)
...
def load_qwen3_kernel_modules() -> LoadedKernelModules:
    ...
    modules = {
        ...
        "decode": _load_kernel_module("decode_layer"),
        "greedy_sample": _load_kernel_module("greedy_sample"),
        "token_embed": _load_kernel_module("token_embed"),
    }
    decode = modules["decode"]
    greedy_sample = modules["greedy_sample"]
    token_embed = modules["token_embed"]
    return LoadedKernelModules(
        functions={
            ...
            "token_embed_fwd": token_embed.token_embed_fwd,
        },
        constants={
            ...
            "token_embed_batch": int(token_embed.BATCH),
            "token_embed_vocab": int(token_embed.VOCAB),
            "token_embed_hidden": int(token_embed.HIDDEN),
        },
    )
...
def validate_qwen3_kernel_modules(
    ...
    _expect(constants, "decode_seq_tile", QWEN3_14B_TILING.seq_tile, "decode_layer SEQ_TILE mismatch")
    _expect(constants, "decode_block_size", QWEN3_14B_TILING.block_size, "decode_layer BLOCK_SIZE mismatch")
    _expect(constants, "decode_batch", kernel_batch, "decode_layer.decode_fwd fixed BATCH mismatch")
    _expect(constants, "decode_num_layers", int(config.num_hidden_layers), "decode_layer NUM_LAYERS mismatch")
    _expect(constants, "decode_vocab", padded_vocab, "decode_layer VOCAB mismatch")
    _expect(constants, "decode_real_vocab", int(config.vocab_size), "decode_layer REAL_VOCAB mismatch")
    ...
    _expect(constants, "token_embed_batch", kernel_batch, "token_embed_fwd fixed BATCH mismatch")
    _expect(constants, "token_embed_vocab", padded_vocab, "token_embed_fwd VOCAB mismatch")
    _expect(constants, "token_embed_hidden", int(config.hidden_size), "token_embed_fwd HIDDEN mismatch")
...
def get_qwen3_14b_serving_contract() -> ModelServingContract:
    ...
    return ModelServingContract(
        ...
        kernels={
            ...
            "token_embed": _TOKEN_EMBED_STAGE,
        },
...
_TOKEN_EMBED_STAGE = KernelSpec(
    name="token_embed",
    public_name="qwen3_14b.token_embed_fwd",
    args=(
        _arg("sampled_ids", "int32", ("B", "SAMPLED_IDS_PAD")),
        _arg("embed_weight", "bf16", ("VOCAB", "H")),
        _arg("next_hidden", "bf16", ("B", "H"), "out"),
    ),
    host_jit_fn=qwen3_token_embed_host,
    compile_args_builder=build_token_embed_compile_args,
    runtime_args_builder=build_token_embed_runtime_args,
)
